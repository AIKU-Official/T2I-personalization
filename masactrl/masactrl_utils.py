...
def regiter_attention_editor_diffusers(model, editor: AttentionBase):
    ...
    def ca_forward(self, place_in_unet):
        def forward(x, encoder_hidden_states=None, attention_mask=None, context=None, mask=None):
            ...
            """
            # Shape of x : [b * 4, 4096, 320]
            # b[0] : uncond source image, b[1] : uncond target image, b[2] : source, b[3] : target
            ...
            k = self.to_k(context)
            v = self.to_v(context)
            q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (q, k, v)) # Become [b * 4 * 8, n, d]
