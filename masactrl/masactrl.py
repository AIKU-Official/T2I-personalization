...
class MutualSelfAttentionControl(AttentionBase):
    ...
    def __init__(self, start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50, model_type="SD", total = False, tep = None):
        """
    ...
    def attn_batch(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        """
        b = q.shape[0] // num_heads # 2 (source and target)
        q = rearrange(q, "(b h) n d -> h (b n) d", h=num_heads) # [8, b * 1024, 80]
        k = rearrange(k, "(b h) n d -> h (b n) d", h=num_heads) # [8, b * 512, 80]
        v = rearrange(v, "(b h) n d -> h (b n) d", h=num_heads) # [8, b * 512, 80]

        sim = torch.einsum("h i d, h j d -> h i j", q, k) * kwargs.get("scale") # [8, b * 1024, b * 512]
        attn = sim.softmax(-1)
        out = torch.einsum("h i j, h j d -> h i d", attn, v) # [8, b * 1024, 80]
        out = rearrange(out, "h (b n) d -> b n (h d)", b=b) # [b, 1024, 8 * 80]
        return out
...
class MutualSelfAttentionControlMask(MutualSelfAttentionControl):
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        H = W = int(np.sqrt(q.shape[1]))
        qu, qc = q.chunk(2) # order : uncond_source, uncond_target, cond_source, cond_target
        ku, kc = k.chunk(2)
        vu, vc = v.chunk(2)
        attnu, attnc = attn.chunk(2)
        ...
        # Use Q_target, K_source, V_source
...
class MutualSelfAttentionControlMaskAuto(MutualSelfAttentionControl):
    def __init__(self, start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50, thres=0.1, ref_token_idx=[1], cur_token_idx=[1], mask_save_dir=None, model_type="SD", total=False):
        """
        ...
        Args:
            ...
            mask_save_dir: the path to save the mask image
        """
        super().__init__(start_step, start_layer, layer_idx, step_idx, total_steps, model_type, total=total)
